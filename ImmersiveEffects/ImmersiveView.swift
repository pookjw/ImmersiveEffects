...
struct ImmersiveView: View {
    private let selectedEffect: Effect?
    @State private var buffer: ManagedBuffer<Void, Effect?> = .create(minimumCapacity: 1, makingHeaderWith: { buffer in
        buffer.withUnsafeMutablePointerToElements { ptr in
            ptr.initialize(to: nil)
        }
    })
    
    init(selectedEffect: Effect?) {
    ...
    var body: some View {
        GeometryReader3D { geometry in
            ...
            .onChange(of: selectedEffect, initial: true) { oldValue, newValue in
                buffer.withUnsafeMutablePointerToElements { ptr in
                    ptr.pointee = selectedEffect
                }
            }
        }
    }
    ...
    private func updateEffects(content: RealityViewContent, sceneFrame: BoundingBox) {
        let isEqual: Bool = buffer.withUnsafeMutablePointerToElements { ptr in
            let result: Bool = ptr.pointee == selectedEffect
            ptr.pointee = selectedEffect
            return result
        }
